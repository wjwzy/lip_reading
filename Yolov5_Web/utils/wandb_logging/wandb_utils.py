...
from Yolov5_Web.utils.general import colorstr, xywh2xyxy
...
class WandbLogger():
    ...
    def setup_training(self, opt, data_dict):
        ...
        if self.train_artifact_path is not None:
            train_path = Path(self.train_artifact_path) / 'web_data/images/'
            data_dict['train'] = str(train_path)
        if self.test_artifact_path is not None:
            test_path = Path(self.test_artifact_path) / 'web_data/images/'
    ...
    def download_dataset_artifact(self, path, alias):
        if path.startswith(WANDB_ARTIFACT_PREFIX):
            ...
            labels_zip = Path(datadir) / "web_data/labels.zip"
            shutil.unpack_archive(labels_zip, Path(datadir) / 'web_data/labels', 'zip')
    ...
    def log_dataset_artifact(self, dataset, class_to_id, name='dataset'):
        ...
        artifact.add_dir(image_path, name='web_data/images')
        ...
        artifact.add_file(str(zip_path), name='web_data/labels.zip')
        wandb.log_artifact(artifact)
        print("Saving web_data to W&B...")
