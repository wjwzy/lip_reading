...
from LipReading.data_process import logging
...
class ResNet(nn.Module):
    def __init__(self, c_in, num_class):
        ...
        self.pool3d1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=(0, 0, 0))

        self.conv3d2 = nn.Conv3d(64, 128, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn2 = nn.BatchNorm3d(128)
        self.relu = nn.ReLU(inplace=True)
        self.pool3d2 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 1, 1), padding=(0, 0, 0))
    ...
    def forward(self, inputs, targets=None):
        ...
        if self.show_log: print('GRU:', out.size())  #(bsz, time_step, 2*hidden_size)
...
if __name__ == '__main__':
    lip_net = ResNet(c_in=3, num_class=100)
    a = torch.randn(4, 3, 24, 112, 112)
    out = lip_net(a, None)
    print(out.size())

    # onnx_path = "onnx_model_name.onnx"
    # torch.onnx.export(lip_net, a, onnx_path)
    #
